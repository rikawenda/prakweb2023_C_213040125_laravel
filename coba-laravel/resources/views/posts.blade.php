...
@section('container')
    ...
    <div class="container">
      <div class="row">
        ...
        <div class="card">
          <div class="position-absolute px-3 py-2 text-white" style="background-color: rgba(179, 179, 179, 0.8)"><a href="/posts?category={{ $post->category->slug }}" class="text-white text-decoration-none">{{ $post->category->name }}</a>
        </div>
        @if($post->image)
        ...
        @endif
            <div class="card-body">
              ...
              <p>
                <small class="text-muted">
                  By. <a href="/posts?author={{ $post->author->username }}" class="text-decoration-none"> {{ $post->author->slug }}</a> {{ $post->created_at->diffForHumans() }}
                </small>
                </p>
